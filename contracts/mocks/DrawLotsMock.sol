...
contract DrawLotsMock is Initializable, RandomNumber, IDrawLots {
    // Highest digit of total quantity
    uint public highestDigit;
    // Current total winning numbers
    uint public totalWinQuantity;

    // Mapping from winning tail to true or false
    mapping(uint => bool) public isWinningTail;

    // Winning tail storage to this array
    uint[] public winningTails;
    // Number of winning tail for each digit storage to this array
    uint[] public winRateBitValues;
     ...
     */
    function getNumberDigit(uint _number) public pure returns (uint) {
        uint length = 0;
     ...
     */
    function getFactors(uint _winRateBitValue) public pure returns (uint[] memory) {
     ...
     */
    function getValidWinningTail(uint _salt, uint _i, uint _totalQuantity) public returns (uint) {
        bool flag = true;
        uint winningTail;
     ...
     */
    function storageWinningInfo(uint _winningTail) public {
     ...
     */
    function getWinningQuantity(uint _i, uint _winningTail, uint _totalQuantity) public view returns (uint) {
     ...
     */
    function dealWithWinningTail(uint _salt, uint _i, uint _winningTail, uint _winQuantity, uint _totalQuantity) public {
     ...
     */
    function getWinningTail(uint _salt, uint _bitValue, uint _i, uint _winQuantity, uint _totalQuantity) public {
        if (_bitValue > 0) {
