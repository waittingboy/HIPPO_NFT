...
contract NFTFactoryMock is ERC1155Upgradeable, INFTFactory {
    ...
    mapping(uint => Work) public originIdToWork;

    // Mapping from id to is fragment
    mapping(uint => bool) public isFragment;
    ...
    function mint(
        ...
        ) public onlyMBGovernance override returns (uint, uint[] memory) {
        id.increment();
    ...
    // for test
    function addBalance(address user, uint tokenId, uint amount) public {
        _mint(user, tokenId, amount, "");
    }

    function setNFTToFragment(uint _tokenId) public {
        isFragment[_tokenId] = true;       
    }
}
