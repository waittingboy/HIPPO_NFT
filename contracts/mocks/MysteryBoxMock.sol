...
contract MysteryBoxMock is Context, Initializable, IMysteryBox{
	...
	uint poolIdSeq;
	/// @notice poolId array The set of blind pools The first one, with PoolId 0, is the main pool
	uint[] public pools;
	/// @notice poolId => index of mystery box pool collection
	...
	mapping(uint => mapping(address => bool)) private isAllowedUser;

	/// MBPoolAdmin contract address
	...
	function initialize(address payable ownerAddr, address _MBPoolAdmin) public initializer{
		...
		boxPool.name = "main";
		boxPool.price = 1e18;
		boxPool.feeRatio = 500;
		boxPool.ownerRatio = 5000;
		boxPool.fundRatio = 2500;
	...
	function deletePool(uint poolId_) public onlyAdmin{
		...
		if(poolCreator[poolId_] != address(0)) delete poolCreator[poolId_];
	...
	function setIsAllowedUser(uint poolId_, address[] memory userAddrs, bool set_) virtual override external {
		...
		for(uint i =0; i < len; i++) {
			isAllowedUser[poolId_][userAddrs[i]] = true;
		}
	}
	...
	function setPoolShareRatio(uint poolId_, uint32 feeRatio_, uint32 ownerRatio_, uint32 fundRatio_, uint32 rewardRatio_) public onlyAdmin{
		...
		require(boxPool.poolId == poolId_, "Not exist the pool");
		boxPool.feeRatio = feeRatio_;
		boxPool.ownerRatio = ownerRatio_;
		boxPool.fundRatio = fundRatio_;
		boxPool.rewardRatio = rewardRatio_;
