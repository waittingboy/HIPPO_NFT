...
contract NFTAuctionMarketMock is ERC721HolderUpgradeable, ERC1155HolderUpgradeable, ERC20Tokens {
    ...
    uint[] public biddableAuctionItemIds;
    // Mapping from biddable auction item id to its index
    mapping(uint => uint) public biddableAuctionItemIdIndex;
     ...
     */
    function deleteNFTAuctionItem(uint _auctionItemId) public {
    ...
    function getBiddableAuctionItemIds() public view returns(uint[] memory tmpAuctionItemIds) {
        uint _len = getBiddableAuctionItemQuantity();
        tmpAuctionItemIds = new uint[](_len);
        tmpAuctionItemIds = biddableAuctionItemIds;
    }
    ...
    function getAuctioneerAuctionItemIds() public view returns(uint[] memory tmpAuctionItemIds) {
        uint _len = getAuctioneerAuctionItemQuantity(_msgSender());
        tmpAuctionItemIds = new uint[](_len);
        tmpAuctionItemIds = auctioneerAuctionItemIds[_msgSender()];
    }
    ...
    function getTokenAuctionItemIds(uint _tokenId) public view returns(uint[] memory tmpAuctionItemIds) {
        uint _len = getTokenAuctionItemQuantity(_tokenId);
        tmpAuctionItemIds = new uint[](_len);
        tmpAuctionItemIds = tokenIdOfAuctionItemIds[_tokenId];
    }
    ...
    function getBidderAuctionItemIds() public view returns(uint[] memory tmpAuctionItemIds) {
        uint _len = getBidderAuctionItemQuantity(_msgSender());
        tmpAuctionItemIds = new uint[](_len);
        tmpAuctionItemIds = bidderAuctionItemIds[_msgSender()];
    }
    ...
    function getBidderRecords(uint _auctionItemId) public view returns(BidRecord[] memory tmpAuctionItemIds) {
        uint _len = getAuctionItemBidRecordQuantity(_auctionItemId);
        tmpAuctionItemIds = new BidRecord[](_len);
        tmpAuctionItemIds = bidRecords[_auctionItemId];
    }
    ...
    function getNFTAuctionItem(uint _auctionItemId) public view returns (NFTAuctionItem memory) {
        return auctionItemIdOfAuctionItem[_auctionItemId];
    }

    function isBidderOfAuctionItem(uint _auctionItemId) public view returns(bool) {
        return isBidAuctionItem[_msgSender()][_auctionItemId]; 
    }
}
