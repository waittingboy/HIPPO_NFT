...
contract Democracy is IDemocracy, OwnableUpgradeable {
    ...
    struct ProposalInfo {
        ...
        uint256 valuation; // uint USD, token amount with decimals
    ...
    function initialize(
        ...
        usdToken = _usdToken;
        minValuation = 1 * 10 ** usdToken.decimals();
    ...
    function initProposal(
        ...
        uint256 len = _ibase.getCollectLen(_collectionId);
        require(len > 0, "Nums of works need GT 0");
        require(
    ...
    function _initProposal(
        ...
        uint256 totalPropFee = uploadFeePerWork * _len;
        ...
        emit Proposal(_uploader, _collectionId, _len, 0);
        return true;
    }
    ...
    function modifyProposal(
        ...
        uint256 len = _base.getCollectLen(_proposalId);
        require(len > 0, "Nums of works need GT 0");
        uint256 totalPropFee = uploadFeePerWork * len;
        hipToken.safeTransferFrom(_msgSender(), feeTo, totalPropFee);

        _setPropInfo(_proposalId, _crowdfundRatio, _dividendRatio, _duration, _maxJoinAmount, _valuation);

        emit Proposal(_msgSender(), _proposalId, len, proposal.failNum);
    }
    ...
    function toCrowdfund(uint256 _proposalId) public {
        ...
        uint256 _targetAmount = getTargetAmount(proposal.valuation);
        ...
        if(_openAmount % hipAmountPerHnq != 0) {
            _openAmount = (_openAmount/hipAmountPerHnq +1)*hipAmountPerHnq;
        }
    ...
    returns (uint256)
    {
        uint256 targetAmount = oracle.getTargetAmount(_valuation);
        return targetAmount * pledgeRate / 10000;
    }
    ...
    function canCrowdfund(uint256 _proposalId) public override view returns (bool) {
        ...
        return crowdfundStatus == 3 ? true : false;
    }
