...
contract ExternalNftBase is IExternalNftBase, OwnableUpgradeable {
    using SafeERC20Upgradeable for IERC20Upgradeable;

    IDemocracy private democracy;
    ...
    modifier onlyMysteryBoxMarket() {
        require(_msgSender() == mysteryBoxMarket, "Only mysteryBoxMarket can call");
    ...
    function _createMbPackage(uint256 _collectionId,  uint256 _poolId) internal {
        Collection storage collection = collections[_collectionId];
    ...
    function createMbPackageOneStep(
        ...
        require(_whichOp == 2,"Must call _createMbPackage");
    }

    /// @notice Create mystery Box package and put it into exclusive pool one step
    /// @param _tokenAddresses the token addresses of NFTs which will be put into mystery box package
    /// @param _tokenIds the tokenIds of NFTs which will be put into mystery box package
    /// @param _amounts the tokenIds' amounts of NFTs which will be put into mystery box package
    /// @param _collectionName the collection name will be created
    /// @param _collectionDesc the collection description will be created
    /// @param _collectionUrl the metadata url of collection which will be created
    /// @param _poolId the pool id of exclusive pool
    function createMbPackageAndPutIntoExclusivePool(
        ...
        uint256 _whichOp = _createMbPackageOneStep(_collectionId,0,_endIndex,_poolId);
        require(_whichOp == 2,"Must call _createMbPackage");
    ...
    function poolingBonusToCollectOwner(
        ...
        for (uint256 i = 0; i <= lastAccIndex; i++) {
            ...
            if (transferAmountThis > 0) {
                IERC20Upgradeable(_bonusToken).safeTransferFrom(
                    _msgSender(),
                    _creator,
                    transferAmountThis
                );
    ...
    // for test
    function setMysteryBoxMarket(address _mysteryBoxMarket) public onlyOwner {
        require(_mysteryBoxMarket != address(0),"Not 0x00");
        mysteryBoxMarket = _mysteryBoxMarket;
    }
